...
pub const Color = enum {
    ...
    yellow,
    noColor, 
    multi, //Used for some actions cards like joker, essentially matches any color


    ///Returns true if col1 and col2 are compatible, 
    ///meaning that a color card with col1 can be played on col2  
...
pub const Card = struct {
    ...
    priority: u32 = 3, //A card with a high prio is played earlier

    const ServerJokerValue = 1;
    const LocalJokerValue = 4;

    //Used for sorting cards by their priority before making the final move 
    ...
    ///Returns true if the color is compatible with this card
    ...
    ///Sets this cards prio in respect to both the last top card and the opponents hand size.
...
///Collection of cards currently in the hand of a player
...
pub const GameState = struct {
    ...
    ///Given the current top card, returns the card that the player is make their move for 
    pub fn getPlayableTopCard(self: *const GameState) Card {
        return blk: {
            ...
            if (self.top_card.cardType == CardType.view) {
                ...
                //If a view is placed on top of a view, 
                //we interpret the top card as having a number of one with the color set to the 
                //same as the lowest view card
...
//Construcst a move object with a single card and sends it to the server
...
///Given a set of cards and the current hand of the player, calculates which card is the least likely 
///to cause us to lose cards with the next turn if the opponent does not play a card.
...
pub fn chooseMoveCards(alloc: std.mem.Allocator, move: *Move, deck: []Card, playable_cards: []Card, count: u32, opponent_hand_size: usize) !void {
    ...
    //
    //Index of the card that will be the new top card next turn
    ...
    //We play risky if our opponent has a high amount of cards. If we have very few cards, we play safe.  
    ...
    //Playing safe means that we always put the lowest risk card at the top of the cards that we play.
    if (playSafe) {
...
//Given seperate decks of cards that only contain playable cards, returns the one with the highest summed priority 
...
//Main point where we construct a turn 
pub fn makeMove(alloc: std.mem.Allocator, game_state: *const GameState, socket: *sio.SocketIO, event: *const sio.EventData) !void {
    ...
    if (defs.chooseColorBySumOfPrio) {
        ...
        //If we don' want the summed prio approach, just return the bigger deck
...
//Iterates through hand by provided filters.
